...
try:
    ...
    os.environ['EGO_MASTER_LIST'] = "gz-master.test.ibm.com"
...
autoStart="%s">%s</sc:Dependency> ' % (self.properties[self.CONTROL_POLICY][self.DEPENDENCY][self.DTYPE],
    ...
    def _get_final(self):
        return ' <ego:ExecutionUser>egoadmin</ego:ExecutionUser> \
    ...
    def _resolve_attribute(self, name):
        import pdb; pdb.set_trace()
        if name == self.IPv4:
            search_opts = {'device_id': self.properties[self.SVC_NAME]}
